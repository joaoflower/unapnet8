...
<?
	...
	if(fsafetylogin())
	{
		...
		if(!empty($_POST['rNum_rec']) and !empty($_POST['rImp_pag']) and !empty($_POST['rHra_sec']) and !empty($_POST['rFch_pag']))
		{
			...
			{
				if($aResult['secuencia'] == $_POST['rNum_rec'] and $aResult['imp_pag'] == $_POST['rImp_pag'] and 
					$aResult['fch_pag'] == fFechamy($_POST['rFch_pag']) and $_POST['rImp_pag'] == $_SESSION['sCidimp_pag'] )
				{
